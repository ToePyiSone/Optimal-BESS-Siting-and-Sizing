...
"""
# Add this function to simulate regulation market activation
def simulate_reg_activation(hour: int, reg_bid: dict) -> tuple:
    ...
    # Additional probability based on bid sizes
    up_prob = base_prob * (reg_bid.get('bid_up', 0) > 0)  # Use .get() to avoid KeyError
    down_prob = base_prob * (reg_bid.get('bid_down', 0) > 0)  # Use .get() to avoid KeyError
...
# Modified scenario function with detailed revenue tracking (example for pv_grid_scenario)
def process_regulation_service(bess_state, reg_bid, hour, market_price, ops, results_tracker):
    ...
    if reg_bid['bid_up'] > 0 or reg_bid['bid_down'] > 0:
        ...
        # Handle regulation down activation
        if down_activated and reg_bid['bid_down'] > 0:
            reg_down_power = min(
                reg_bid['bid_down'],
                reg_capacity['reg_down']
            )
...
def perform_sensitivity_analysis(base_results, top_projects, latest_results_pv_only, latest_results_pv_grid):
    ...
    sensitivity_params = {
   'CAPEX': {'variations': [-20, -10, 0, 10, 20, 30]},  # Investment cost uncertainty
   'OPEX': {'variations': [-15, -7.5, 0, 7.5, 15, 25]},  # Operating cost variation
   'MARKET_PRICES': {'variations': [-30, -15, 0, 15, 30, 45]},  # Market price volatility
   'FIT_PRICES': {'variations': [-15, -7.5, 0, 7.5, 15, 20]},  # Feed-in tariff changes
   'DISCOUNT_RATE': {'variations': [-4, -2, 0, 2, 3, 4]},  # In percentage points
   'EFFICIENCY': {'variations': [-5, -2.5, 0, 2.5, 5]},  # Technical efficiency
   'PV_GENERATION': {'variations': [-15, -7.5, 0, 7.5, 15, 20]}  # Resource uncertainty
}
...
# -----------------------------
# Data for PV-only (EEG) Scenario
# -----------------------------
projects_eeg = ['Bus 18', 'Bus 13', 'Bus 14', 'Bus 15', 'Bus 7']

# CAPEX and NPV are in million €, while OPEX and Annual Net Revenue are in thousand €
capex_eeg = np.array([1.27, 3.62, 3.50, 2.18, 1.79])   # M€
npv_eeg   = np.array([3.05, 1.96, 2.72, 1.95, 1.75])   # M€
opex_eeg  = np.array([25.35, 72.45, 70.08, 43.51, 35.86])  # k€
ann_net_eeg = np.array([381.46, 263.78, 456.18, 287.86, 251.69])  # k€

# -----------------------------
# Data for PV+Grid (Merchant) Scenario
# -----------------------------
projects_merchant = ['Bus 5', 'Bus 8', 'Bus 30', 'Bus 7', 'Bus 14']

capex_merchant = np.array([2.55, 3.08, 1.34, 2.53, 1.74])  # M€
npv_merchant   = np.array([3.13, 2.58, 3.25, 3.18, 3.04])  # M€
opex_merchant  = np.array([50.98, 61.50, 26.86, 50.52, 34.84])  # k€
ann_net_merchant = np.array([317.56, 275.56, 296.33, 347.29, 331.41])  # k€


def plot_scenario(projects, capex, npv, opex, ann_net, scenario_title):
    x = np.arange(len(projects))
    width = 0.35

    # Create a figure with two subplots: one for CAPEX/NPV and one for OPEX/Annual Net Revenue.
    fig, (ax_top, ax_bottom) = plt.subplots(2, 1, figsize=(10,8), sharex=True)

    # --- Top subplot: CAPEX and NPV (in million €) ---
    # For clarity, CAPEX and NPV are in M€, so they have similar scales.
    bars_capex = ax_top.bar(x - width/2, capex, width, label='CAPEX (M€)', color='lightcoral', edgecolor='k')
    bars_npv   = ax_top.bar(x + width/2, npv, width, label='NPV (M€)', color='limegreen', edgecolor='k')
    ax_top.set_ylabel('Value (Million €)')
    ax_top.set_title(scenario_title + ' - CAPEX & NPV')
    ax_top.axhline(0, color='grey', linewidth=0.8)
    ax_top.legend()
    
    # Annotate bars for top plot (optional)
    for i in range(len(x)):
        ax_top.text(x[i] - width/2, capex[i] + 0.05, f'{capex[i]:.2f}', ha='center', va='bottom', fontsize=9)
        ax_top.text(x[i] + width/2, npv[i] + 0.05, f'{npv[i]:.2f}', ha='center', va='bottom', fontsize=9)

    # --- Bottom subplot: OPEX and Annual Net Revenue (in thousand €) ---
    bars_opex = ax_bottom.bar(x - width/2, opex, width, label='OPEX (k€)', color='darkorange', edgecolor='k')
    bars_ann = ax_bottom.bar(x + width/2, ann_net, width, label='Annual Net Revenue (k€)', color='skyblue', edgecolor='k')
    ax_bottom.set_ylabel('Value (k€)')
    ax_bottom.set_title(scenario_title + ' - OPEX & Annual Net Revenue')
    ax_bottom.axhline(0, color='grey', linewidth=0.8)
    ax_bottom.legend()
    
    # Annotate bars for bottom plot (optional)
    for i in range(len(x)):
        ax_bottom.text(x[i] - width/2, opex[i] + 2, f'{opex[i]:.1f}', ha='center', va='bottom', fontsize=9)
        ax_bottom.text(x[i] + width/2, ann_net[i] + 5, f'{ann_net[i]:.1f}', ha='center', va='bottom', fontsize=9)
    
    # Set common x-axis labels
    plt.xticks(x, projects)
    ax_bottom.set_xlabel('Project')
    plt.tight_layout()
    plt.show()


# Plot for PV-only (EEG) Scenario
plot_scenario(projects_eeg, capex_eeg, npv_eeg, opex_eeg, ann_net_eeg, 'PV-only (EEG) Scenario')

# Plot for PV+Grid (Merchant) Scenario
plot_scenario(projects_merchant, capex_merchant, npv_merchant, opex_merchant, ann_net_merchant, 'PV+Grid (Merchant) Scenario')



import numpy as np

def calculate_metrics(capex, annual_net, project_names):
    # Convert annual_net from k€ to M€ to match CAPEX units
    annual_net_m = annual_net / 1000
    
    # Calculate ROI (%)
    roi = (annual_net_m / capex) * 100
    
    # Calculate Payback Period (years)
    payback = capex / annual_net_m
    
    # Print results in a formatted table
    print("\n{:<12} {:<15} {:<12} {:<15}".format(
        "Project", "CAPEX (M€)", "ROI (%)", "Payback (years)"))
    print("-" * 55)
    
    for i, project in enumerate(project_names):
        print("{:<12} {:<15.2f} {:<12.1f} {:<15.1f}".format(
            project, capex[i], roi[i], payback[i]))
    
    return roi, payback

# Calculate for PV-only (EEG) scenario
print("\nPV-only (EEG) Scenario Results:")
print("=" * 55)
projects_eeg = ['Bus 18', 'Bus 13', 'Bus 14', 'Bus 15', 'Bus 7']
roi_eeg, payback_eeg = calculate_metrics(capex_eeg, ann_net_eeg, projects_eeg)

# Calculate summary statistics for EEG scenario
print("\nEEG Scenario Summary:")
print(f"Average ROI: {np.mean(roi_eeg):.1f}%")
print(f"Average Payback Period: {np.mean(payback_eeg):.1f} years")
print(f"ROI Range: {np.min(roi_eeg):.1f}% - {np.max(roi_eeg):.1f}%")
print(f"Payback Range: {np.min(payback_eeg):.1f} - {np.max(payback_eeg):.1f} years")

# Calculate for PV+Grid (Merchant) scenario
print("\nPV+Grid (Merchant) Scenario Results:")
print("=" * 55)
roi_merchant, payback_merchant = calculate_metrics(capex_merchant, ann_net_merchant, projects_merchant)

# Calculate summary statistics for Merchant scenario
print("\nMerchant Scenario Summary:")
print(f"Average ROI: {np.mean(roi_merchant):.1f}%")
print(f"Average Payback Period: {np.mean(payback_merchant):.1f} years")
print(f"ROI Range: {np.min(roi_merchant):.1f}% - {np.max(roi_merchant):.1f}%")
print(f"Payback Range: {np.min(payback_merchant):.1f} - {np.max(payback_merchant):.1f} years")

# Create comparison of scenarios
print("\nScenario Comparison:")
print("=" * 55)
print(f"{'Metric':<20} {'EEG':<15} {'Merchant':<15}")
print("-" * 55)
print(f"{'Average ROI':<20} {np.mean(roi_eeg):.1f}% {np.mean(roi_merchant):.1f}%")
print(f"{'Average Payback':<20} {np.mean(payback_eeg):.1f} years {np.mean(payback_merchant):.1f} years")
print(f"{'Best ROI':<20} {np.max(roi_eeg):.1f}% {np.max(roi_merchant):.1f}%")
print(f"{'Best Payback':<20} {np.min(payback_eeg):.1f} years {np.min(payback_merchant):.1f} years")
...
# -----------------------------
# Example Data for PV-only (EEG) Scenario
# -----------------------------
projects_eeg = ['Prenzlau', 'Kleinwusterwitz', 'Gahro-2', 'Serbitz', 'Vahldorf']
roi_eeg = np.array([30.0, 7.3, 13.0, 13.2, 14.1])   # in %
payback_eeg = np.array([3.3, 13.7, 7.7, 7.6, 7.1])  # in years

# -----------------------------
# Example Data for PV+Grid (Merchant) Scenario
# -----------------------------
projects_merchant = ['Kleinwusterwitz', 'Ronneburg', 'Bresewitz', 'Vahldorf', 'Gahro-2']
roi_merchant = np.array([12.5, 9.0, 22.1, 13.7, 19.0])  # in %
payback_merchant = np.array([8.0, 11.2, 4.5, 7.3, 5.3]) # in years

def plot_roi_payback_dual_axis(ax, projects, roi, payback, scenario_title):
    x = np.arange(len(projects))
    width = 0.4
    ...
    # Plot ROI bars (left axis)
    bars_roi = ax.bar(x - width/2, roi, width, color='green', alpha=0.7, label='ROI (%)')
    # Plot Payback bars (right axis)
    bars_payback = ax2.bar(x + width/2, payback, width, color='blue', alpha=0.7, label='Payback (years)')

    # Set labels and title
    ax.set_ylabel('ROI (%)', color='green')
    ax2.set_ylabel('Payback (years)', color='blue')
    ax.set_title(scenario_title, fontsize=12, pad=10)

    # Position projects on x-axis
    ax.set_xticks(x)
    ax.set_xticklabels(projects, rotation=0)

    # Turn off all grid lines
    ax.grid(False)
    ax2.grid(False)

    # Remove top and right spines on the primary axis
    for spine in ["top", "right"]:
        ax.spines[spine].set_visible(False)
    # Remove top spine on the secondary axis (keep right spine for payback scale)
    ax2.spines["top"].set_visible(False)

    # Annotate values on top of each bar (optional)
    for bar in bars_roi:
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2, height + 0.5,
                f'{height:.1f}%', ha='center', va='bottom', color='green', fontsize=9)
    for bar in bars_payback:
        height = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2, height + 0.3,
                 f'{height:.1f}', ha='center', va='bottom', color='blue', fontsize=9)
    ...
    # Adjust y-limits to add some space for labels
    ax.set_ylim(0, max(roi)*1.2)
    ax2.set_ylim(0, max(payback)*1.2)


# Create a figure with two subplots (one for each scenario)
fig, (ax_top, ax_bottom) = plt.subplots(2, 1, figsize=(9, 8))

# Plot PV-only (EEG) scenario on the top subplot
plot_roi_payback_dual_axis(ax_top, projects_eeg, roi_eeg, payback_eeg, 'PV-only Scenario')

# Plot PV+Grid (Merchant) scenario on the bottom subplot
plot_roi_payback_dual_axis(ax_bottom, projects_merchant, roi_merchant, payback_merchant, 'PV+Grid Scenario')
...
# -----------------------------
projects_eeg = ['Prenzlau', 'Kleinwusterwitz', 'Gahro-2', 'Serbitz', 'Vahldorf']
capex_eeg = np.array([2.63, 2.4, 2.06, 2.41, 2.18])  # in million €

# -----------------------------
# Data for PV+Grid (Merchant) Scenario
# -----------------------------
projects_merchant = ['Kleinwusterwitz', 'Ronneburg', 'Bresewitz', 'Vahldorf', 'Gahro-2']
capex_merchant = np.array([3.09, 3.21, 2.325, 2.995, 2.08])  # in million €

# Create a figure with two side-by-side subplots
fig, (ax_left, ax_right) = plt.subplots(1, 2, figsize=(10, 5), sharey=True)

# --- Left subplot: PV-only (EEG) ---
bars_left = ax_left.bar(projects_eeg, capex_eeg, color='lightcoral', edgecolor='k', alpha=0.8)
ax_left.set_title('EEG Scenario', fontsize=12, pad=10)
ax_left.set_ylabel('CAPEX (Million €)')
ax_left.set_ylim(0, max(capex_eeg.max(), capex_merchant.max()) * 1.2)  # Same scale on both subplots
ax_left.set_xticklabels(projects_eeg, fontsize=9)
...
# --- Right subplot: PV+Grid (Merchant) ---
bars_right = ax_right.bar(projects_merchant, capex_merchant, color='skyblue', edgecolor='k', alpha=0.8)
ax_right.set_title('Merchant Scenario', fontsize=12, pad=10)
ax_right.set_xticklabels(projects_eeg, fontsize=9)
# We share the y-axis, so no need to set it again here
...
# Remove unnecessary spines/grids for a cleaner look
...
def plot_weekly_bid_blocks_fixed(results_dict, scenario_type, top_projects, start_hour=4344):
    """
    Plot weekly bidding amounts with correct price variations
    """
    plt.style.use('seaborn-v0_8-whitegrid')
    ...
    # Time range for one week (168 hours)
    hours = range(start_hour, start_hour + 168)
    x_hours = range(168)
    
    # Get data for the top performing project
    scenario_key = 'pv_only' if scenario_type == 'PV-only' else 'pv_grid'
    top_project = top_projects[scenario_key][0]
    project_idx = top_project['rank'] - 1
    results_tracker = results_dict['results'][project_idx]['results_tracker']
    
    # Extract data and make sure to use actual price data
    da_bids = results_tracker.p_discharge[hours]
    reg_up_bids = results_tracker.p_reg_up[hours]
    
    # Get actual price data from hourly_data
    market_prices = hourly_data['Price_Euro_per_MWh'].iloc[hours].values
    reg_up_prices = hourly_data['Reg_up'].iloc[hours].values
    reg_down_prices = hourly_data['Reg_down'].iloc[hours].values
    
    # Filter out zero bids
    da_nonzero = [(h, b) for h, b in enumerate(da_bids) if b > 0.01]
    reg_up_nonzero = [(h, b) for h, b in enumerate(reg_up_bids) if b > 0.01]
    
    # Plot price lines with actual variations
    line1 = ax2.plot(x_hours, market_prices, color='#f1c40f', 
                     alpha=0.7, linewidth=1.5, label='Market Price')
    line2 = ax2.plot(x_hours, reg_up_prices, color='#3498db',
                     alpha=0.3, linewidth=1, label='Reg Up Price')
    line3 = ax2.plot(x_hours, reg_down_prices, color='#e74c3c',
                     alpha=0.3, linewidth=1, label='Reg Down Price')
    
    # Plot non-zero bids
    if da_nonzero:
        x_da, y_da = zip(*da_nonzero)
        scatter1 = ax1.scatter(x_da, y_da, marker='s', s=50, 
                             color='#2ecc71', alpha=0.7, label='Day-ahead Bids')
        # Add dots on market price line for DA bids
        ax2.scatter(x_da, [market_prices[x] for x in x_da], 
                   color='#2ecc71', s=30, zorder=5)
    
    if reg_up_nonzero:
        x_reg_up, y_reg_up = zip(*reg_up_nonzero)
        scatter2 = ax1.scatter(x_reg_up, y_reg_up, marker='^', s=50,
                             color='#3498db', alpha=0.7, label='Regulation Up Bids')
        # Add dots on reg up price line
        ax2.scatter(x_reg_up, [reg_up_prices[x] for x in x_reg_up],
                   color='#3498db', s=30, zorder=5)
    
    if scenario_type == 'PV+Grid':
        reg_down_bids = results_tracker.p_reg_down[hours]
        reg_down_nonzero = [(h, b) for h, b in enumerate(reg_down_bids) if b > 0.01]
        if reg_down_nonzero:
            x_reg_down, y_reg_down = zip(*reg_down_nonzero)
            scatter3 = ax1.scatter(x_reg_down, y_reg_down, marker='v', s=50,
                                 color='#e74c3c', alpha=0.7, label='Regulation Down Bids')
            # Add dots on reg down price line
            ax2.scatter(x_reg_down, [reg_down_prices[x] for x in x_reg_down],
                       color='#e74c3c', s=30, zorder=5)
    
    # Calculate week number and season
    week_num = start_hour//168 + 1
    month = (week_num * 7) // 30 + 1
    season = "Summer" if 6 <= month <= 8 else "Winter" if month in [12, 1, 2] else "Spring" if 3 <= month <= 5 else "Fall"
    ...
    # Labels and title
    ax1.set_xlabel('Hours', fontsize=12)
    ax1.set_ylabel('Bid Amount (MW)', fontsize=12)
    ax2.set_ylabel('Price (€/MWh)', fontsize=12)
    plt.title(f'Weekly Bidding Profile - {scenario_type} Scenario\n'
              f'Project {top_project["rank"]} (Bus {top_project["bus_number"]}) - Week {week_num} ({season})',
              fontsize=14, pad=20)
    
    # X-axis formatting
    major_ticks = np.arange(0, 169, 24)
    minor_ticks = np.arange(0, 169, 6)
    ax1.set_xticks(major_ticks)
    ax1.set_xticks(minor_ticks, minor=True)
    ax1.set_xticklabels([f'Day {d//24+1}' for d in major_ticks])
    ax1.grid(True, alpha=0.3)
    
    # Get all plot elements for legend
    legend_elements = []
    legend_labels = []
    
    if 'scatter1' in locals():
        legend_elements.append(scatter1)
        legend_labels.append('Day-ahead Bids')
    if 'scatter2' in locals():
        legend_elements.append(scatter2)
        legend_labels.append('Regulation Up Bids')
    if 'scatter3' in locals():
        legend_elements.append(scatter3)
        legend_labels.append('Regulation Down Bids')
    legend_elements.extend([line1[0], line2[0], line3[0]])
    legend_labels.extend(['Market Price', 'Reg Up Price', 'Reg Down Price'])
    ...
    # Project details
    textstr = (f'BESS: {top_project["bess_power_mw"]:.1f}MW/{top_project["bess_energy_mwh"]:.1f}MWh\n'
               f'Grid: {top_project["grid_connection_mw"]:.1f}MW')
    props = dict(boxstyle='round', facecolor='white', alpha=0.8)
    ax1.text(1.02, 0.02, textstr, transform=ax1.transAxes, fontsize=9,
...
# Plot both scenarios for summer and winter weeks
# Summer week (July)
#plot_weekly_bid_blocks_fixed(latest_results_pv_only, 'PV-only', top_projects, start_hour=4000)
plot_weekly_bid_blocks_fixed(latest_results_pv_grid, 'PV+Grid', top_projects, start_hour=4150)

# Winter week (January)
#plot_weekly_bid_blocks_fixed(latest_results_pv_only, 'PV-only', top_projects, start_hour=24)
plot_weekly_bid_blocks_fixed(latest_results_pv_grid, 'PV+Grid', top_projects, start_hour=24)
#ä##########################################################################
def plot_weekly_bid_blocks_with_buy(results_dict, scenario_type, top_projects, start_hour=4344):
    """
    Plot weekly bidding amounts including buy bids for PV+Grid scenario
    """
    plt.style.use('seaborn-v0_8-whitegrid')
    fig, ax1 = plt.subplots(figsize=(15, 8))
    ax2 = ax1.twinx()
    ...
    # Extract all bid types including charging/buy bids
    da_bids = results_tracker.p_discharge[hours]
    reg_up_bids = results_tracker.p_reg_up[hours]
    charge_grid_bids = results_tracker.p_charge_grid[hours]  # Grid charging bids
    ...
    da_nonzero = [(h, b) for h, b in enumerate(da_bids) if b > 0.01]
    reg_up_nonzero = [(h, b) for h, b in enumerate(reg_up_bids) if b > 0.01]
    charge_grid_nonzero = [(h, b) for h, b in enumerate(charge_grid_bids) if b > 0.01]  # Grid charging
    ...
    # Plot grid charging bids
    if charge_grid_nonzero:
        x_charge, y_charge = zip(*charge_grid_nonzero)
        scatter4 = ax1.scatter(x_charge, [-b for b in y_charge], marker='D', s=50,  # Plot below x-axis
                             color='#9b59b6', alpha=0.7, label='Grid Charging Bids')
        ax2.scatter(x_charge, [market_prices[x] for x in x_charge],
                   color='#9b59b6', s=30, zorder=5)
    ...
    if 'scatter3' in locals():
        legend_elements.append(scatter3)
        legend_labels.append('Regulation Down Bids')
    if 'scatter4' in locals():
        legend_elements.append(scatter4)
        legend_labels.append('Grid Charging Bids')
    legend_elements.extend([line1[0], line2[0], line3[0]])
    legend_labels.extend(['Market Price', 'Reg Up Price', 'Reg Down Price'])
...
# Plot scenarios
plot_weekly_bid_blocks_with_buy(latest_results_pv_grid, 'PV+Grid', top_projects, start_hour=4150)
plot_weekly_bid_blocks_with_buy(latest_results_pv_grid, 'PV+Grid', top_projects, start_hour=24)
...
print("\nBase Case NPV Analysis for PV Projects")
print("=" * 80)

for result in base_results:
    print(f"\nBus {result['bus_number']}:")
    print("-" * 40)
    print(f"PV Capacity: {result['pv_capacity_mw']:.2f} MW")
    print(f"Grid Connection: {result['grid_connection_mw']:.2f} MW")
    print(f"FiT Price: {result['fit_price']:.2f} €/MWh")
    print(f"Annual Revenue: €{result['annual_revenue']:,.2f}")
    print(f"Annual O&M Cost: €{result['annual_om_cost']:,.2f}")
    print(f"Net Annual Profit: €{result['net_annual_profit']:,.2f}")
    print(f"NPV (10-year): €{result['npv']:,.2f}")
...
# Data for PV-only scenario
pv_only_base_npv = {
    18: 3.225122980,  # Bus: NPV in millions
    13: 2.171383830,
    14: 1.969473840,
    15: 1.842676910,
    7: 1.811730270
...
# Data for PV-only annual profits
pv_only_base_profit = {
    18: 359.70013,  # Bus: Profit in thousands
    13: 242.17589,
    14: 219.65674,
    15: 205.51499,
    7: 202.06349
...
# Data for PV+Grid annual profits
pv_grid_base_profit = {
    5: 257.34198,
    8: 213.43351,
    30: 208.01615,
...
def plot_comparison(ax, base_data, bess_data, y_label, title):
    buses = list(base_data.keys())
    x = np.arange(len(buses))
    width = 0.35

    base_values = [base_data[bus] for bus in buses]
    bess_values = [bess_data[bus] for bus in buses]

    # Calculate percentage changes
    percent_changes = [(bess - base) / abs(base) * 100 for base, bess in zip(base_values, bess_values)]

    # Create bars
    bars1 = ax.bar(x - width/2, base_values, width, label='Base Case', color='#2ecc71', alpha=0.7)
    bars2 = ax.bar(x + width/2, bess_values, width, label='With BESS', color='#3498db', alpha=0.7)

    # Add percentage change labels
    for i, (pct, bess_val) in enumerate(zip(percent_changes, bess_values)):
        y_pos = max(base_values[i], bess_val) + (max(base_values) * 0.02)
        ax.text(i, y_pos, f'{pct:+.1f}%', ha='center', va='bottom')
...
# Plot NPV comparisons
plot_comparison(ax1, pv_only_base_npv, pv_only_bess_npv, 'NPV (M€)', 'PV_ONLY: NPV Comparison')
plot_comparison(ax2, pv_grid_base_npv, pv_grid_bess_npv, 'NPV (M€)', 'PV_GRID: NPV Comparison')

# Plot Annual Profit comparisons
plot_comparison(ax3, pv_only_base_profit, pv_only_bess_profit, 'Annual Profit (k€)', 'PV_ONLY: Annual Profit Comparison')
plot_comparison(ax4, pv_grid_base_profit, pv_grid_bess_profit, 'Annual Profit (k€)', 'PV_GRID: Annual Profit Comparison')
...
def plot_combined_scenario(hourly_data, reg_prices, top_projects, start_hour=0, end_hour=168):
    # Use a professional style for academic publications
    plt.style.use('seaborn-v0_8-whitegrid')
    
    # Create figure with higher DPI for better print quality
    fig, ax = plt.subplots(figsize=(12, 7), dpi=300)
    
    # Slice data
    market_slice = hourly_data['Price_Euro_per_MWh'][start_hour:end_hour]
    reg_up_slice = reg_prices['Reg_up'][start_hour:end_hour]
    reg_down_slice = reg_prices['Reg_down'][start_hour:end_hour]
    
    hours = np.arange(len(market_slice))  # X-axis (0..168 if a week)
    
    # Plot main market lines with academic-friendly colors
    ax.plot(hours, market_slice, label='Day-Ahead Price', color='#004488', 
            linewidth=2.0, zorder=10)
    ax.plot(hours, reg_up_slice, label='Regulation Up', color='#22AA99', 
            linewidth=2.0, zorder=10)
    ax.plot(hours, reg_down_slice, label='Regulation Down', color='#AA3377', 
            linewidth=2.0, zorder=10)
    
    # Define line style & color for each scenario
    scenario_styles = {
        'pv_only': {'linestyle': '--', 'linewidth': 1.5, 'alpha': 0.9, 'zorder': 5},
        'pv_grid': {'linestyle': '-.', 'linewidth': 1.5, 'alpha': 0.9, 'zorder': 5}
    }
    
    # Color palettes for each scenario - using colorblind-friendly palettes
    palettes = {
        'pv_only': sns.color_palette("Paired", len(top_projects.get('pv_only', []))),
        'pv_grid': sns.color_palette("Dark2", len(top_projects.get('pv_grid', [])))
    }
    
    # Add horizontal FiT lines for each scenario with distinct colors
    for scenario, projects in top_projects.items():
        style_kwargs = scenario_styles.get(scenario, {'linestyle': ':', 'linewidth': 1.5})
        for i, proj in enumerate(projects):
            fit_val = proj['fit_price']
            proj_name = proj['name']
            
            # Get color from appropriate palette
            color = palettes[scenario][i]
            
            ax.axhline(
                y=fit_val,
                label=f"FiT - {proj_name}",
                color=color,
                **style_kwargs
            )
    
    # Title & axis labels with professional font
    ax.set_title("Market Price vs. Feed-in Tariff Comparison", 
                fontsize=16, fontweight='bold', fontfamily='serif')
    ax.set_xlabel("Hours", fontsize=12, fontfamily='serif')
    ax.set_ylabel("Price (€/MWh)", fontsize=12, fontfamily='serif')
    
    # Add day markers on x-axis for better readability
    days = [24*i for i in range(8)]  # 0, 24, 48, ..., 168
    day_labels = [f"Day {i}" for i in range(8)]
    ax.set_xticks(days)
    ax.set_xticklabels(day_labels, fontsize=10, fontfamily='serif')
    
    # Add minor ticks every 6 hours for more precision
    minor_ticks = np.arange(0, end_hour-start_hour, 6)
    ax.set_xticks(minor_ticks, minor=True)
    
    # Thousands separator on y-axis
    ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, p: format(int(x), ',')))
    
    # Light grid with better visibility
    ...
    # Add light gray background shading for day/night (optional)
    for i in range(0, len(hours), 24):
        # Light shading for night hours (6PM-6AM)
        evening_start = i + 18  # 6PM
        if evening_start < len(hours):
            evening_end = min(i + 24 + 6, len(hours))  # 6AM next day or end
            ax.axvspan(evening_start, evening_end, alpha=0.05, color='gray', zorder=1)
    
    # Get legend handles and labels
    handles, labels = ax.get_legend_handles_labels()
    
    # Group legend items by category: first 3 for market, then FiT lines.
    market_handles = handles[:3]
    market_labels = labels[:3]
    
    fit_handles = handles[3:]
    fit_labels = labels[3:]
    
    # Create Market legend and place it above the FiT legend.
    market_legend = ax.legend(market_handles, market_labels, 
                              loc='upper left', bbox_to_anchor=(1.02, 1), 
                              framealpha=0.9, title="Market Prices",
                              fontsize=9)
    ax.add_artist(market_legend)
    
    # Create the FiT legend and place it below the Market legend.
    fiT_legend = ax.legend(fit_handles, fit_labels, 
             loc='upper left', bbox_to_anchor=(1.02, 0.8), 
             borderaxespad=0, title="Feed-in Tariffs",
             fontsize=9, ncol=1)
...
# ---------------- USAGE EXAMPLE ----------------
# Example: Suppose you have a dictionary like this:
top_projects = {
    'pv_only': [
        {'name': 'Prenzlau',       'fit_price': 91.9},
        {'name': 'Kleinwusterwitz','fit_price': 72.9},
        {'name': 'Gahro 2',        'fit_price': 220.7},
        {'name': 'Serbitz',        'fit_price': 89.2},
        {'name': 'Vahldorf',       'fit_price': 100.9},
        {'name': 'Ronneburg',      'fit_price': 109.3},
        {'name': 'Bresewitz',      'fit_price': 64.2},
    ]
}

# Then call the function:
fig, ax = plot_combined_scenario(hourly_data, reg_prices, top_projects, start_hour=4344, end_hour=4512)

# Uncomment to save as high-resolution image for thesis
#plt.savefig('market_fit_comparison.png', dpi=300, bbox_inches='tight')
#plt.savefig('market_fit_comparison.pdf', bbox_inches='tight')  # Vector format for publication
